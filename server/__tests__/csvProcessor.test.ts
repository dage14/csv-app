...
describe("CSVProcessor", () => {
  ...
  describe("processFile()", () => {
    ...
    test("throws error for empty file", async () => {
      await expect(processor.processFile(emptyFilePath)).rejects.toThrow(
        "Error processing CSV: Empty file"
      );
    }, 10000);

    test("throws error for malformed CSV", async () => {
      await expect(processor.processFile(malformedFilePath)).rejects.toThrow(
        "Error processing CSV"
      );
    });

    test("throws error for non-existent file", async () => {
      await expect(processor.processFile("nonexistent.csv")).rejects.toThrow(
        "ENOENT"
      );
    });
  });
  ...
  describe("getProcessedFilePath()", () => {
    ...
    test("handles path traversal attempts", () => {
      expect(() => processor.getProcessedFilePath("../malicious.csv")).toThrow(
        "Invalid filename"
      );
    });
  });
